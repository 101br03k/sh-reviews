...
router.get('/', (req, res) => {
  db.all('SELECT * FROM reviews ORDER BY created_at DESC', [], (err, rows) => {
    if (err) {
      console.error(err);
      return res.render('index', { reviews: [], error: 'Error retrieving reviews' });
    }
...
router.post('/submit', upload.single('image'), (req, res) => {
  ...
  if (!username || !review) return res.redirect('/');
  db.run('INSERT INTO reviews (username, review, image) VALUES (?, ?, ?)', [username, review, image], function(err) {
    if (err) {
      console.error(err);
      return res.redirect('/');
    }
